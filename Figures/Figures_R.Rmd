...
```{r}
ggplot(result[result$figure == "Fig_3n",], aes(x = node, y = AUC)) + 
...
```{r}
tmp <- result[result$figure == "Fig_4f",]
...
ggplot(result[result$figure == "ED_Fig_9f",], aes(x = node, y = AUC)) + 
  geom_boxplot() + 
...
tmp <- result[result$figure == "ED_Fig_9g",]
tmp <- pivot_longer(tmp, cols = c("AUC",  "Accuracy", "Sensitivity", "Specificity", "Fscore"), names_to = "measure", values_to = "value")
tmp$measure <- factor(tmp$measure, levels = c("AUC",  "Accuracy", "Sensitivity", "Specificity", "Fscore"))


ggplot(tmp[tmp$figure == "ED_Fig_9g",], aes(x = node, y = value)) + 
  geom_boxplot() + 
  geom_point(aes(colour = node), size = 2) +
  facet_grid(cols = vars(measure)) + 
  xlab("node") + 
